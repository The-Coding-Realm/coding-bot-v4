    ...
    try:
        ...
        try:
            ...
            try:
                ...
                await ctx.send(f'I couldnt find `{target}`')
                return None
    return target
...
class Config(commands.Cog):
    ...
    async def _prefix(self, ctx, *prefixes):
        ...
        if prefixes == []:
            ...
            embed.add_field(name='To reset:',value=f'`{ctx.prefix + ctx.command.qualified_name} default` to reset the prefix to default, or replace default with your own prefix')
            return await ctx.send(embed=embed)
        prefixes = prefixes or ['set to default']
        embed = ctx.embed(title='Successfully changed prefix', description=f'Prefixes are now `{"`, `".join(prefixes)}``' if len(prefixes) > 1 else f'Prefix is now {prefixes[0]}')
    ...
    async def disabled(self, ctx):
        ...
        content = '\n'.join([(await convert(ctx, command)).mention + ': ' + (('`' + '`, `'.join(commands[command]) + '`.') if not True in commands[command] else 'All Commands') for command in commands if commands.get(command, None)]) or 'No Commands are disabled'
    ...
    async def _disable_command(self, ctx, command, *, target=None):
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        return await ctx.send(f'I couldnt find `{target}`')
        ...
        if not command:
            return await ctx.send(embed=ctx.error('That isnt a command!'))
        if command.qualified_name.split()[0] in ['config', 'help']:
            return await ctx.send(embed=ctx.error("You cannot disable the config or help commands."))
        embed = ctx.embed(title='Command Disabled', description=f'I will now ignore the command {command.qualified_name}{the_for}.')

        async with self.bot.pools.config.acquire() as connection:
            data = await connection.fetchrow('SELECT * FROM serverconf WHERE id = $1', ctx.guild.id)
            if not data:
                commands = {
                    str(target.id): [command.qualified_name]
                }
                await connection.execute('INSERT INTO serverconf (id, commands) VALUES ($1, $2::json)', ctx.guild.id, commands)
            else:
                ...
                if command.qualified_name in commands[str(target.id)]:
                    return await ctx.send(embed=ctx.error(f'`{command.qualified_name}` is already disabled{the_for}.'))
                commands[str(target.id)].append(command.qualified_name)
                await connection.execute('UPDATE serverconf SET commands = $1 WHERE id = $2', commands, ctx.guild.id)
        await self.bot.helpers.prepare(self.bot, ctx.guild)
        await ctx.send(embed=embed)
    ...
    async def _disable_all(self, ctx, *, target=None):
        if not target is None:
            ...
            target = ctx.channel
        the_for = ' for ' + target.mention
        embed = ctx.embed(title='Commands Disabled', description=f'I will now ignore all commands{the_for}.')
        ...
        async with self.bot.pools.config.acquire() as connection:
            ...
            if not data:
                ...
                if True in commands[str(target.id)]:
                    return await ctx.send(embed=ctx.error(f'All commands are already disabled{the_for}.'))
                commands[str(target.id)].append(True)
                await connection.execute('UPDATE serverconf SET commands = $1::json WHERE id = $2', commands, ctx.guild.id)
        await self.bot.helpers.prepare(self.bot, ctx.guild)
        await ctx.send(embed=embed)
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        return await ctx.send(embed=ctx.error(f'I couldnt find `{target}`'))
        else:
            target = ctx.channel
        the_for = ' for ' + target.mention
        embed = ctx.embed(title='Commands Enabled', description=f'I will now stop ignoring all commands{the_for}.')

        async with self.bot.pools.config.acquire() as connection:
            data = await connection.fetchrow('SELECT * FROM serverconf WHERE id = $1', ctx.guild.id)
            if not data:
                return await ctx.send(embed=ctx.error(f'Commands are not disabled{the_for}.'))
            else:
                commands = data['commands']
                if isinstance(commands, dict):
                    commands[str(target.id)] = commands.get(str(target.id), [])
    ...
    async def _enable_command(self, ctx, command, *, target=None):
        ...
        try:
            ...
            return await ctx.send(embed=ctx.error('That isnt a command!'))
        embed = ctx.embed(title='Command Enabled', description=f'I will now stop ignoring the command {command.qualified_name}{the_for}.')

        async with self.bot.pools.config.acquire() as connection:
            data = await connection.fetchrow('SELECT * FROM serverconf WHERE id = $1', ctx.guild.id)
            if not data:
                return await ctx.send(embed=ctx.error(f'`{command.qualified_name}` is not disabled{the_for}.'))
            else:
                commands = data['commands']
                if isinstance(commands, dict):
                    commands[str(target.id)] = commands.get(str(target.id), [])
